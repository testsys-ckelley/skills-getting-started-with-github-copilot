      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = participants.length
          ? `<ul class="participants-list">${participants
              .map(
                (email) =>
                  `<li class="participant-item"><span>${email}</span><button type="button" class="participant-remove" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Remove ${email} from ${name}" title="Unregister participant">&times;</button></li>`
              )
              .join("")}</ul>`
